...
def create_model(path):
    # defining a list "model" to append the counts of unigrams and bigrams
    model = []
    ...
    for l in f.readlines():
        ...
        for word in tokens:
            ...
            for i in range(len(word)):
                unigrams[word[i]] += 1
                # checking till the last but one character
                if i < (len(word) - 1):
    ...
    for l in f.readlines():
        ...
        for word in tokens:
            ...
            for i in range(len(word) - 1):
                prob1 = float(b[word[i]][word[i + 1]] + 1) / (a[word[i]] + 27)
...
def main(en_tr, es_tr, folder_te):
    ...
    print "Prediction for French documents in test:"
